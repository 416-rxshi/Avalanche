...
class Shadowshape():
    ...
    def update(self,shape):
        ...
        while not self.atbottom:
            for i in range(4):
                if self.bottomblocklist[i].gridyPosition == gameboardHeight - 1 or activeBoardspot[self.bottomblocklist[i].gridxPosition][self.bottomblocklist[i].gridyPosition + 1]:
                    self.atbottom=True
            for i in range(self.numblocks):
